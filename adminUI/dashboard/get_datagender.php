...
$gender = isset($_GET['gender']) ? $_GET['gender'] : ''; // รับค่า gender จาก query string
...
try {
    ...
    // กำหนด parameter สำหรับ gender
    $params = [];
    $types = "";

    if (!empty($gender)) {
        $sql .= " WHERE s.gender = ?"; // เปลี่ยนเป็นกรองตาม gender
        $params[] = $gender;
        $types .= "s"; // กำหนดชนิดข้อมูลเป็น string
    }
