...
$year = isset($_GET['year']) ? $_GET['year'] : ''; // รับค่า year จาก query string

try {
    // คำสั่ง SQL ดึงข้อมูลตามปีการศึกษาที่เลือก
    ...
    // ถ้ามีการเลือกปีการศึกษา (year) จะเพิ่มเงื่อนไข WHERE
    $params = [];
    $types = "";

    if (!empty($year)) {
        $sql .= " WHERE s.year = ?"; // ใช้ปีการศึกษาแทนสาขา
        $params[] = $year;
        $types .= "i";  // ระบุชนิดข้อมูลที่ใช้ใน parameter (i = integer)
    }
